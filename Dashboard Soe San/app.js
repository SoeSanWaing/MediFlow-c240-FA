...
function isToday() {
  return filterDate === new Date().toISOString().slice(0, 10);
}
  ...
  if (patient.waitTimeMinutes !== null && patient.waitTimeMinutes !== undefined && patient.waitTimeMinutes !== '') {
    ...
    if (assignedTime) {
      ...
      if (remaining <= 0) {
        ...
        return overdue === 0 ? 'Due now' : `Due now (${overdue} min overdue)`;
      }
      return `${remaining} min wait`;
    }
    return `${patient.waitTimeMinutes} min wait`;
  }
...
function renderDashboard(records) {
  ...
  patientCount.textContent = `Total: ${filteredRecords.length} patient${filteredRecords.length !== 1 ? 's' : ''}`;
    ...
    list.className = 'patient-list';
      ...
      meta.innerHTML = `
        ...
        ${isToday() ? `<span>${formatWaitDisplay(patient)}</span>` : ''}
      ...
      // Only show action buttons for today's patients
      if (isToday()) {
...
async function fetchPatientData() {
  ...
  try {
    // Send date parameter so n8n can filter by date
    const fetchUrl = filterDate ? `${DATA_URL}?date=${filterDate}` : DATA_URL;
